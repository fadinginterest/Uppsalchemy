...
namespace Uppsalchemy
{
	public static class ModConfig
	{
		...
		static T LoadConfig<T>(ICoreAPI api, string jsonConfig) where T : IModConfig
		{
			return api.LoadModConfig<T>(jsonConfig);
		}

		static void GenerateConfig<T>(ICoreAPI api, string jsonConfig, T previousConfig = null) where T : class, IModConfig
		{
			api.StoreModConfig(CloneConfig(api, previousConfig), jsonConfig);
		}

		static T CloneConfig<T>(ICoreAPI api, T config = null) where T : class, IModConfig
		{
